...
(define (part-1)
  ...
  (let ((larger 1000000))
    ...
    ;; (show-coords)
    (format #t "finding nulls ....~%")
    (find-nulls)
    (format #t "expanding universe ....~%")
    (expand-universe)
    ;;(arrange-top-bottom-left-right)
    ;; (format #t "-----after exapnsions------~%")
    ;; (show-coords)
    
    (format #t "computing shortest paths ....~%")
    (shortest-paths)  
    ))
...
*expansion-rate* to be million times larger (one less than additive to empty row column)
