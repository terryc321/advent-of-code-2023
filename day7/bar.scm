...
amend the joker hand
so it gets the best classification possible

A     K   Q   J  10  9  8  7  6  5  4  3  2
14   13   12  *  10  9  8  7  6  5  4  3  2
...
#|
rather than try set joker card to try every possibility

(define (classify-joker hand)
  (let ((best '(0 dummy)))  
    (define (test zs)
      (let ((cls (classify zs)))
	(when (> (first (first cls)) (first best))
	  (set! best (first cls)))))
    (define (make-hand xs ys)
      (cond
       ((null? xs) (test ys))
       (#t (let ((c1 (car xs)))
	     (cond
	      ((eqv? c1 11) (dolist (c '(14 13 12 10 9 8 7 6 5 4 3 2))
				    (make-hand (cdr xs) (cons c ys))))
	      (#t (make-hand (cdr xs) (cons c1 ys))))))))
    (make-hand hand '())
    (format #t "best joker hand for [~a] was [~a] ~%" hand best)
    (list best)))
|#


#|
when try categorise hand
only looking to make cards

|#
(define (most-common-non-joker-card hand)
  (let ((rs (map (lambda (n) (filter (lambda (x) (= x n)) hand))
		 '(2 3 4 5 6 7 8 9 10 12 13 14))))
    (let ((longest
	   (first (sort rs (lambda (x y) (> (length x) (length y)))))))
      (cond
       ((null? longest) 2)
       (#t (first longest))))))
    ...
    (define (make-hand xs ys)
      (cond
       ...
       (#t (let ((c1 (car xs)))
	     (cond
	      ((eqv? c1 11) ;;(dolist (c '(14 13 12 10 9 8 7 6 5 4 3 2))
	       (let ((c (most-common-non-joker-card hand)))
		 (make-hand (cdr xs) (cons c ys))))
	      (#t (make-hand (cdr xs) (cons c1 ys))))))))
    (make-hand hand '())
    ;;(format #t "best joker hand for [~a] was [~a] ~%" hand best)
   ...
   (#t
    (let ((c1 (car hand1))
      ...
      (cond
       ...
       ((eqv? c1 11) #f) ;; hand1 has joker , worth least 
       ((eqv? c2 11) #t) ;; hand2 has joker , 
       ((> c1 c2) #t) ;; default to normal numerical comparison 
       (#t #f))))))
...
scheme@(#{ g8590}#) [21]> (part-2)

luckily there was not much else to this 
