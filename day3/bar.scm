...
(* 467 35)    16345     ...... upper right
(* 755 598)   451490    ...... lower right

find a symbol * that is adjacent to exactly two numbers 
...
(define (scan-asterisk-symbols in)
  (let* (;;(in (process xs))
	 (lim (vector-length in))
	 (j 0)
	 (k 0)
...
(define (get-parts in)
  (let* ((flats (flat-compat-lis (scan-digits in)))
	 (parts '()))
    (do-list (flat flats)
	     (cond
	      ((part-number? flat in)
	       (set! parts (cons flat parts)))))
    ;;(set! parts (reverse parts))
    parts))

(define (find-adjacent-parts x y lim-x lim-y parts)
  (let ((res '()))
    (do-list (part parts)
	     (match part
	       ((val (x1 y1)(x2 y1)) 
		;; duplicated code !!! we love it !!
		(call/cc (lambda (escape)
		    (do-for (j (- x1 1) (+ x2 1 1))
			     (do-for (k (- y1 1) (+ y1 1 1))
				      (cond
				       ((and (>= j 0) (< j lim-x)
					     (>= k 0) (< k lim-y)
					     (= j x) (= k y))
					(set! res (cons part res))
					(escape #t)))))				    
		    #f)))
	       (_ (error "find-adjacent-parts no match"))))
    ;;(set! res (reverse res))
    res))
...
(define (foo dat)
  (let* ((in (process dat))
	 (lim-x (vector-length (vector-ref in 0)))
	 (lim-y (vector-length in))
	 (parts (get-parts in))
	 (stars (scan-asterisk-symbols in))
	 (sum 0))
    (do-list (star stars)
	     (let ((tmp '()))
	     (match star 
	       ((ch x y) 
		(let ((adj (map first (find-adjacent-parts x y lim-x lim-y parts))))		  
		;;(set! res  (cons (list star ) res))	       
		  (cond
		   ((= (length adj) 2)
		    ;;(format #t "adj = ~a ~%" adj)
		    (let ((prod (apply * adj)))
		      (set! sum (+ sum prod)))))))
	       (_ (error "foo no match")))))    
    sum))



(define (example-1) (foo (example)))
(define (part-2) (foo (input)))
...
#;7312> ,t (part-2)
9.383s CPU time, 0.198s GC time (major), 19301618/244678 mutations (total/tracked), 54/113861 GCs (major/minor), maximum live heap: 4.64 MiB
